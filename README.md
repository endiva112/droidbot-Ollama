...
# Descargar APK a la carpeta apks/
adb pull <ruta_del_apk> apks/app.apk
```

**Ejemplo con la app Reloj:**
```bash
adb shell pm path com.google.android.deskclock
adb pull /product/app/PrebuiltDeskClockGoogle/PrebuiltDeskClockGoogle.apk apks/reloj.apk
```

### 4. Ejecutar DroidBot
```bash
python start.py -d <dispositivo> -a apks/<apk> -policy llm_guided -count 30 -o informes/<nombre_informe>
```

**Ejemplo:**
```bash
python start.py -d emulator-5554 -a apks/reloj.apk -policy llm_guided -count 30 -o informes/reloj_test1
```
...
Los resultados se guardan en la carpeta `informes/<nombre_informe>/`:
